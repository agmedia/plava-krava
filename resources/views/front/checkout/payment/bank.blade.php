<form name="pay" class="needs-validation w-100" action="{{ route('checkout') }}" novalidate  method="GET">
    ...
    <div class="form-check form-check-inline">
        <label class="form-check-label" for="ex-check-4">{!! __('Prije kupnje potrebno je prihvatiti :terms_of_service', [
                                                'terms_of_service' => '<a target="_blank" href="'.route('catalog.route.page',['page' => 'opci-uvjeti-koristenja-i-privatnosti']).'" class="link-fx">'.__('Opći uvjete korištenja i privatnosti').'</a>',
